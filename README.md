...
- 생성되는 답변을 실시간으로 표시하고 완료 후 토큰 사용량 안내
- 리뷰 세션 동안 최근 20개 카드의 대화와 작성 중인 질문 복원
- 마지막 질문 재시도
- 저장된 API 키를 설정 입력창에 다시 노출하지 않는 안전한 키 변경·삭제
- 전체 대화 복사와 더 친절한 API 오류 안내
- 스크롤 가능한 카드 수정 전후 비교 및 전체 선택·해제
...
- **빠른 조작**: `⌘+L`은 현재 탭의 입력창에 포커스를 두고, 요청 중 `Esc`는 생성을 취소합니다. Windows/Linux에서는 `Ctrl+L`을 사용합니다.
